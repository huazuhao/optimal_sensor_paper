...
alpha = 0.000; % Control penalty parameter, this parameter originates from the objective function
heat_constant = 1; %The constant in the heat equation. 
project_rank = 5;
rank_cut_off = 10; %for observability gramian rank
...
for iteration_index = 1:10
    ...
    random_norm = norm(U0-U_init_recover_random_access,'fro');
    ...
    for index = 1:n+2    
        ...
        if next_potential_placement(index) ~= 0
            ...
            s_matlab = s_matlab(1:rank_cut_off,1:rank_cut_off);
    ...
    og_norm = norm(U0-U_init_recover_og_access,'fro');
