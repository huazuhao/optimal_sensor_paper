...
fprintf('  Final Objective Value Based On First Order Method: % 8.6e\n',f)
...
%%
%here, we want to recover the initial condition by using the newton method
%implemented by ourselves. 

state.is_state_computed = false;
state.is_adjoint_computed = false;
state.obs_op= random_selected_obs;
...
state.Mobs = Mobs;
gtol = 1e-5;
U_init_recover_newton = newton_method(gtol, state);

f = reducedObj_partial(U_init_recover_newton,state);
fprintf('  Final Objective Value: % 8.6e\n',f)

figure,
plot(x,U_init_recover_newton,'r-','LineWidth',3), hold on
plot(x,U0,'k--','LineWidth',3), hold off
legend('Computed-with Newton','True','Location','NorthEast')
xlabel('x','FontSize',20)
ylabel('U_0','FontSize',20)
set(gca,'FontSize',16)
print('-depsc2','results-newton.eps')
